      ...
      <div className="flex items-center gap-2 sm:gap-3 shrink-0">
        ...
        <div className="relative" ref={dropdownRef}>
          <button
            ...
            className="flex h-10 shrink-0 items-center gap-[10px] rounded-lg border border-[#dbe6ff] dark:border-[#2A3446] bg-white dark:bg-[#1A2233] px-3.5 text-[13px] font-semibold text-[#17307a] dark:text-white hover:bg-[#fcfdff] transition-all shadow-sm focus:outline-none focus:ring-0 focus:border-[#dbe6ff] dark:border-[#2A3446] focus-visible:outline-none focus-visible:ring-0 active:border-[#dbe6ff] dark:border-[#2A3446] active:ring-0"
          >
            <svg
              viewBox="0 0 24 24"
              className="h-[18px] w-[18px] text-[#17307a] dark:text-white shrink-0"
              fill="none"
              stroke="currentColor"
              strokeWidth="2.2"
              strokeLinecap="round"
            ...
            <ChevronDown className={cn("h-3.5 w-3.5 text-[#17307a] dark:text-white shrink-0 transition-transform duration-200", isDropdownOpen && "rotate-180")} />
          </button>

          {isDropdownOpen && (
            <div className="absolute left-0 top-full z-50 mt-2 w-[165px] rounded-xl border border-[#e4ecff] bg-white dark:bg-[#1A2233] p-2.5 shadow-[0_12px_30px_rgba(23,48,122,0.12)]">
              ...
              <div className="relative mb-2">
                ...
                <input
                  ...
                  className="h-[36px] w-full rounded-lg border border-[#7fa5f7] bg-white dark:bg-[#1A2233] pl-9 pr-3 text-[12.5px] text-[#17307a] dark:text-white placeholder-[#8ea0c7] outline-none transition-all focus:border-[#4d82f3] focus:ring-1 focus:ring-[#4d82f3]"
                  autoFocus
                />
              </div>
              ...
              <div className="max-h-[175px] overflow-y-auto pr-0.5 [scrollbar-width:thin] [&::-webkit-scrollbar]:w-[4px] [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-blue-100 [&::-webkit-scrollbar-thumb]:rounded-full">
                {filteredCities.length > 0 ? (
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        city === selectedCity 
                          ...
                          : "text-[#17307a] dark:text-white hover:bg-[#f2f6ff] dark:bg-[#0B0F19]"
                      )}
                    >
                      {city}
                    </button>
                  ...
                  <div className="py-3 text-center text-[11px] font-normal text-[#17307a] dark:text-white">
                    No cities found
                  </div>
                )}
              </div>
      ...
      <div className="flex items-center gap-[7px] sm:gap-[12px] shrink-0 ml-auto lg:ml-0">
        {topNavIcons.map(({ icon: Icon, badge, label, href }) => (
          <button
            ...
            className={cn(
              "relative h-9 w-9 lg:h-10 lg:w-10 shrink-0 flex items-center justify-center rounded-full bg-white dark:bg-[#1A2233] text-[#17307a] dark:text-white shadow-sm ring-1 ring-[#e5ecfb]",
        ...
        {user ? (
          <div className="relative group ml-[5px]">
            <button className="flex h-9 lg:h-10 shrink-0 items-center gap-2 rounded-full border border-[#dbe6ff] dark:border-[#2A3446] bg-white dark:bg-[#1A2233] p-0.5 lg:py-1 lg:pl-1.5 lg:pr-3 hover:bg-[#fcfdff] transition-all shadow-sm focus:outline-none">
              ...
              <span className="hidden text-[13px] font-semibold text-[#17307a] dark:text-white lg:block">Hi, {user.name}</span>
              <ChevronDown className="hidden h-4 w-4 text-[#17307a] dark:text-white lg:block group-hover:rotate-180 transition-transform duration-200" />
            </button>
            <div className="absolute right-0 top-full pt-2 w-48 opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto transition-all duration-200 z-50">
              <div className="bg-white dark:bg-[#1A2233] border border-[#e4ecff] rounded-xl shadow-lg p-1.5">
                <button
                  onClick={() => {
                    const path = window.location.pathname;
                  ...
                  className="w-full text-left px-3 py-2 text-[13px] font-semibold text-[#1a56db] hover:bg-[#f2f6ff] dark:bg-[#0B0F19] rounded-lg transition-colors border-b border-[#f2f6ff] mb-1"
                >
                  View Profile
                </button>
                <button
      ...
      <Modal isOpen={isWishlistOpen} onClose={() => setIsWishlistOpen(false)} title="Your Wishlist">
        {wishlistItems.length === 0 ? (
          ...
          <div className="space-y-6 max-h-[60vh] overflow-y-auto p-1">
            <div>
              <h3 className="font-bold text-slate-900 dark:text-white mb-3 border-b border-slate-100 dark:border-slate-800 pb-2">Saved Garages</h3>
              {wishlistItems.filter((i) => i.type === 'garage').length === 0 ? (
                <p className="text-slate-500 dark:text-slate-400 text-sm py-2">No saved garages yet.</p>
              ) : (
                <div className="space-y-3">
                  {wishlistItems.filter((i) => i.type === 'garage').map((item) => (
                    <div key={item.id || item.name} className="flex gap-4 p-3 bg-white dark:bg-[#1A2233] rounded-xl border border-slate-100 dark:border-slate-800 shadow-sm items-center">
                      <div className="w-16 h-16 bg-slate-50 dark:bg-[#121826] rounded-lg flex items-center justify-center relative overflow-hidden shrink-0">
            ...
            <div>
              <h3 className="font-bold text-slate-900 dark:text-white mb-3 border-b border-slate-100 dark:border-slate-800 pb-2">Saved Products</h3>
              {wishlistItems.filter((i) => i.type !== 'garage').length === 0 ? (
                <p className="text-slate-500 dark:text-slate-400 text-sm py-2">No saved products yet.</p>
              ) : (
                <div className="space-y-3">
            {wishlistItems.filter((i) => i.type !== 'garage').map((item) => (
              <div key={item.id} className="flex gap-4 p-3 bg-white dark:bg-[#1A2233] rounded-xl border border-slate-100 dark:border-slate-800 shadow-sm items-center">
                <div className="w-16 h-16 bg-slate-50 dark:bg-[#121826] rounded-lg flex items-center justify-center relative overflow-hidden shrink-0">
