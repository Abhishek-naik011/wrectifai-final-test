...
export default function InventoryPage() {
  const inventory = [
    { id: '1', name: 'Oil Filter', desc: 'High performance filter', cat: 'Filters', brand: 'Bosch', part: 'BO-0986', stock: 45, unit: '₹320', total: '₹14,400', status: 'In Stock' },
    { id: '2', name: 'Brake Pads (Front)', desc: 'Ceramic brake pads', cat: 'Brakes', brand: 'Brembo', part: 'BR-FP-2211', stock: 18, unit: '₹1,850', total: '₹33,300', status: 'Low Stock' },
    { id: '3', name: 'Engine Oil 5W-30', desc: 'Fully synthetic', cat: 'Oils & Fluids', brand: 'Castrol', part: 'CA-5W30-4L', stock: 32, unit: '₹950', total: '₹30,400', status: 'In Stock' },
    { id: '4', name: 'Spark Plug', desc: 'Iridium spark plug', cat: 'Ignition', brand: 'NGK', part: 'NGK-ILZKAR7B', stock: 25, unit: '₹280', total: '₹7,000', status: 'Low Stock' },
    { id: '5', name: 'Air Filter', desc: 'Engine air filter', cat: 'Filters', brand: 'Mann', part: 'MA-C26168', stock: 0, unit: '₹650', total: '₹0', status: 'Out of Stock' },
    { id: '6', name: 'Car Battery (55Ah)', desc: 'Maintenance free', cat: 'Electrical', brand: 'Exide', part: 'EX-55MF', stock: 7, unit: '₹5,800', total: '₹40,600', status: 'Low Stock' },
    { id: '7', name: 'Clutch Kit', desc: 'Complete clutch kit', cat: 'Transmission', brand: 'LUK', part: 'LUK-620311500', stock: 11, unit: '₹3,250', total: '₹35,750', status: 'In Stock' },
    { id: '8', name: 'Coolant (1L)', desc: 'Premium radiator coolant', cat: 'Oils & Fluids', brand: 'Gulf', part: 'GF-COOL-1L', stock: 36, unit: '₹210', total: '₹7,560', status: 'In Stock' },
    { id: '9', name: 'Wiper Blade (20")', desc: 'All weather performance', cat: 'Wipers', brand: 'Valeo', part: 'VA-20A', stock: 15, unit: '₹450', total: '₹6,750', status: 'Low Stock' },
    { id: '10', name: 'Brake Disc (Front)', desc: 'Ventilated disc', cat: 'Brakes', brand: 'ATE', part: 'ATE-240134020', stock: 4, unit: '₹2,100', total: '₹8,400', status: 'Low Stock' },
  ];
             ...
             <div className="p-4 border-b border-slate-100 bg-slate-50 flex gap-4">
                <div className="flex-1 bg-white border border-blue-100 rounded-lg p-4 flex justify-between items-center shadow-sm">
                  <div>
                    ...
                    <p className="text-2xl font-black text-[#17307a]">482</p>
                ...
                <div className="flex-1 bg-white border border-green-100 rounded-lg p-4 flex justify-between items-center shadow-sm">
                  <div>
                    ...
                    <p className="text-2xl font-black text-[#17307a]">₹8,45,320</p>
                ...
                <div className="flex-1 bg-white border border-yellow-100 rounded-lg p-4 flex justify-between items-center shadow-sm">
                  <div>
                    ...
                    <p className="text-2xl font-black text-[#17307a]">28</p>
                 ...
                 <tbody className="divide-y divide-slate-100">
                   {inventory.map(item => (
                     <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                       <td className="p-4"><div className="flex gap-3 items-center"><div className="w-10 h-10 rounded bg-slate-100"></div><div><p className="text-sm font-bold text-[#17307a]">{item.name}</p><p className="text-[10px] text-slate-500">{item.desc}</p></div></div></td>
                       <td className="p-4"><p className="text-xs font-medium text-slate-600">{item.cat}</p></td>
                       <td className="p-4"><p className="text-xs font-medium text-slate-600">{item.brand}</p></td>
                       <td className="p-4"><p className="text-xs font-medium text-slate-600">{item.part}</p></td>
                       <td className="p-4 text-center"><p className="text-sm font-bold text-slate-800">{item.stock}</p><p className="text-[10px] text-slate-400">{item.cat === 'Oils & Fluids' ? 'Ltrs' : item.cat === 'Brakes' ? 'Sets' : 'Pcs'}</p></td>
                       <td className="p-4"><p className="text-xs font-medium text-slate-600">{item.unit}</p></td>
                       <td className="p-4"><p className="text-xs font-medium text-slate-600">{item.total}</p></td>
                       <td className="p-4">
                         <span className={`px-3 py-1 rounded-full text-[10px] font-bold ${item.status === 'In Stock' ? 'bg-green-50 text-green-600' : item.status === 'Low Stock' ? 'bg-yellow-50 text-yellow-600' : 'bg-red-50 text-red-600'}`}>{item.status}</span>
                       </td>
             ...
             <Card className="p-5">
               ...
               <div className="flex items-center gap-4 mb-4">
                 <div className="w-24 h-24 rounded-full border-[8px] border-green-500 border-l-yellow-400 border-b-red-500 border-r-blue-500 flex items-center justify-center font-bold text-xl text-[#17307a]">
                   <div className="text-center">482<div className="text-[9px] font-medium text-slate-400 -mt-1">Total Items</div></div>
                 </div>
                 <div className="space-y-2 text-[10px] font-bold text-slate-600 flex-1">
                   <div className="flex items-center justify-between"><span className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-green-500"></div> In Stock</span> <span>342 <span className="text-slate-400 font-normal">(71%)</span></span></div>
                   <div className="flex items-center justify-between"><span className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-yellow-400"></div> Low Stock</span> <span>28 <span className="text-slate-400 font-normal">(6%)</span></span></div>
                   <div className="flex items-center justify-between"><span className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-red-500"></div> Out of Stock</span> <span>7 <span className="text-slate-400 font-normal">(1%)</span></span></div>
                   <div className="flex items-center justify-between"><span className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-blue-500"></div> Reserved</span> <span>105 <span className="text-slate-400 font-normal">(22%)</span></span></div>
                 </div>
               </div>
