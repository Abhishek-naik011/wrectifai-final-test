    ...
    <DashboardShell>
      ...
      <div className="p-6 md:p-8 max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          <div className="flex-1 space-y-4">
            {cartItems.length === 0 ? (
              <Card className="p-8 text-center bg-white dark:bg-[#1A2233] border-slate-100 dark:border-slate-800 rounded-[20px] shadow-sm">
              ...
              cartItems.map((item) => (
                <Card key={item.id} className="p-4 flex flex-col sm:flex-row items-center gap-4 bg-white dark:bg-[#1A2233] border-slate-100 dark:border-slate-800 rounded-[20px] shadow-sm">
                  <div className="w-24 h-24 bg-slate-50 dark:bg-[#121826] rounded-xl flex items-center justify-center shrink-0">
                    {item.img ? (
                  ...
                  <div className="flex items-center gap-4 mt-4 sm:mt-0">
                    <div className="flex items-center gap-3 border border-slate-200 dark:border-slate-700 rounded-full px-3 py-1">
                      <button onClick={() => updateQuantity(item.id, -1)} className="text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white font-bold">-</button>
          ...
          <div className="lg:w-80 shrink-0">
            <Card className="p-6 bg-white dark:bg-[#1A2233] border-slate-100 dark:border-slate-800 rounded-[20px] shadow-sm sticky top-24">
              ...
              <div className="space-y-4 text-sm">
                ...
                <div className="flex justify-between">
                  <span className="text-slate-500 dark:text-slate-400">Tax (10%)</span>
      ...
      <Modal isOpen={isCheckoutModalOpen} onClose={() => setIsCheckoutModalOpen(false)} title="Order Placed">
        <div className="text-center py-6">
          ...
          <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2">Checkout Successful!</h3>
          <p className="text-slate-500 dark:text-slate-400 mb-6">Your order has been placed successfully. You will receive an email confirmation shortly.</p>
